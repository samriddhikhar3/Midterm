...
```
```{r Dropping columns with sale price0, warning=FALSE, results='hide'}
...
variables <- c(
  "sale_price", "number_of_bathrooms", "PricePerSqft","dist_to_nearest_pvt_school", "dist_to_comm_corr", "dist_to_nearest_station", "dist_to_nearest_trail", "med_income", "dist_to_nearest_pool", "frontage", "central_air", "fireplaces", "BasementPresent", "exterior_condition", "PctWhite", "PctBlack", "total_livable_area", "BuildingAge",  "total_livable_area", "PctBachelors", "PctPoverty"
)
...
test_data <- test_data %>%
  mutate(lagPriceError = lag.listw(spatialWeights, Price.Error, NAOK = TRUE))
...
ggplot(data = test_data, aes(lagPriceError, Price.Error)) +
