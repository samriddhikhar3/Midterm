...
```
```{r Dropping columns with sale price=0, warning=FALSE, results='hide'}

# Dropping values where sale price = 0 for better prediction
...
```{r New Variables, warning=FALSE, results='hide'}

# Creating a Price Per SqFt variable

data <- 
  data %>%
  mutate(PricePerSqft = (sale_price/total_livable_area))

# Creating a Building Age variable
...
```
```{r fig.align="center", warning=FALSE, echo = FALSE, fig.width = 14, fig.height=28}
...
st_drop_geometry(Variable) %>% 
  dplyr::select(sale_price, PricePerSqft, year_built, number_of_bathrooms, number_of_bedrooms, number_of_rooms, total_livable_area, med_income, PctWhite,PctBlack, PctHispanic, PctBachelors, PctPoverty, dist_to_comm_corr, dist_to_nearest_landmark, dist_to_nearest_pvt_school, dist_to_nearest_school, dist_to_nearest_station, dist_to_floodplain, dist_to_nearest_college, dist_to_nearest_trail, dist_to_nearest_tenniscourt, dist_to_nearest_playground,) %>% 
  filter(sale_price < 10000000) %>%
